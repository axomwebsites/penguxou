  ...
  loop: function(ctx, canvas, state, playerx, playery, drawpenguin, endgame) {
    ...
    if (window.playervy > 0) {
      window.objects.forEach(p => {
        if (window.playerx + 45 > p.x && window.playerx < p.x + p.w && window.playery + 55 > p.y && window.playery + 45 < p.y + p.h) { window.playervy = -13; window.gamescore++; state.coins++; }
      });
    }
    ...
    drawpenguin(window.playerx, window.playery, 45, 55); ctx.fillStyle = "#95a5a6"; window.objects.forEach(p => ctx.fillRect(p.x, p.y, p.w, p.h));
